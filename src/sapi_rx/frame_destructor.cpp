...
estts::Status frame_destructor::decode_frame(const std::string &frame) {
    ...
    auto resp_object = build_command_object(frame.substr(32));

    command.push_back(resp_object);
...
estts::Status frame_destructor::build_command_objects() {
    try {
        for (;;) {
...
std::vector<estts::command_object *> frame_destructor::destruct_ax25() {
    if (estts::ES_OK != build_command_objects()) {
        return {};
    }
    return this->command;
}